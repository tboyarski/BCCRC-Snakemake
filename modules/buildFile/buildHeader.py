...
def buildHeader(chrTYPE, yamlNAME, jsonNAME, snakeNAME):
    ...
    with open(yamlNAME, "w+") as yamlTARGET:
        ...
        annotationSUFFIX="annotationSUFFIX: .cosmic_annotated.indel_annotated.snp_annotated\n"
        ...
        yamlTARGET.write("\n\n#################################\n# ----- Global Variables ------ #\n#################################\n")
        yamlTARGET.write("#       -- Parameters --        #\n" + shellCallFile + fastqKEEP + annotationSUFFIX + refFILE + chrLIST)
    ...
    with open(snakeNAME, "w+") as snakeTARGET:
        ...
        snakeTARGET.write('        #expand("{outputDIR}/{annotateDIR}/{index[1][tumor]}_{index[1][normal]}.varScan.{varType}.canonical_annotated{annotationSUFFIX}.vcf", outputDIR=config["outputDIR"], annotateDIR=config["annotateDIR"], index=pandas.read_table(config["sampleFILE"], " ").iterrows(), varType=config["varType"], annotationSUFFIX=config["annotationSUFFIX"])\n')
        snakeTARGET.write('        #expand("{outputDIR}/{getVarTableDIR}/{index[1][tumor]}_{index[1][normal]}.varScan.{varType}.canonical_annotated{annotationSUFFIX}.txt", outputDIR=config["outputDIR"], getVarTableDIR=config["getVarTableDIR"], index=pandas.read_table(config["sampleFILE"], " ").iterrows(), varType=config["varType"], annotationSUFFIX=config["annotationSUFFIX"])\n')
        snakeTARGET.write('        # Single: Normal Runs\n')
        snakeTARGET.write('        #expand("{outputDIR}/{annotateDIR}/{samples}.varScan.{varType}.canonical_annotated{annotationSUFFIX}.vcf", outputDIR=config["outputDIR"], annotateDIR=config["annotateDIR"], samples=config["sample"], varType=config["varType"], annotationSUFFIX=config["annotationSUFFIX"])\n')
        snakeTARGET.write('        expand("{outputDIR}/{getVarTableDIR}/{samples}.varScan.{varType}.canonical_annotated{annotationSUFFIX}.txt", outputDIR=config["outputDIR"], getVarTableDIR=config["getVarTableDIR"], samples=config["sample"], varType=config["varType"], annotationSUFFIX=config["annotationSUFFIX"])\n')
#-------------------------------------------------------------------------------
