...
if (os.path.isdir("log/" + moduleNAME) != True):
    ...
    moduleLIST = ['alignBAM_bwa','bam2fastq','cleanBAM','fastq2GZ','fastqc',
            ...
            'rmdupBAM','sam2BAM','sortBAM_biobambam','sortBAM_samtools']
...
with open(sys.argv[2], "r+") as jsonTARGET:
    ...
    jsonOBJ['sam2BAM'] = {
            "clusterSpec": "-V -S /bin/bash -o log/processBam/sam2BAM -e log/processBam/sam2BAM -l h_vmem=10G -pe ncpus 1",
...
with open(sys.argv[3], "a+") as pipeTARGET:
    pipeTARGET.write(
        ...
        '#bam2fastq         ~ expand("{outputDIR}/{fastqDIR}/{samples}.{readDirection}.fastq", outputDIR=config["outputDIR"], fastqDIR=config["fastqDIR"], samples=config["sample"], readDirection=["1", "2"])\n'+
        ...
        '#fastq2GZ          ~ expand("{outputDIR}/{fastqDIR}/{samples}.fastq.gz", outputDIR=config["outputDIR"], fastqDIR=config["fastqDIR"], samples=config["sample"])\n'+
        '#fastqc            ~ expand("{outputDIR}/{fastqDIR}/{samples}.fastq.zip", outputDIR=config["outputDIR"], fastqDIR=config["fastqDIR"], samples=config["sample"])\n'+
        ...
        '#namesortBAM       ~ expand("{outputDIR}/{processBamDIR}/{samples}_namsort.bam", outputDIR=config["outputDIR"], processBamDIR=config["processBamDIR"], samples=config["sample"])\n'+
        ...
        '#sam2BAM           ~ expand("{outputDIR}/{samDIR}/{samples}.sam", outputDIR=config["outputDIR"], samDIR=config["samDIR"], samples=config["sample"])\n'+
