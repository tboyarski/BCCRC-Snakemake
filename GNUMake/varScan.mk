...
#----------
# VCF Fields to Output
# Depending on What Annotations are Set 
#----------
VCF_FIELDS = CHROM POS ID REF ALT QUAL FILTER

VCF_SAMPLES ?= 0
VCF_INFO_FIELDS = 
VCF_GEN_IDS ?= GT GQ SDP DP RD AD FREQ PVAL RBQ ABQ RDF RDR ADF ADR
VCF_GEN_FIELDS = $(foreach gen,$(VCF_GEN_IDS),$(foreach i,$(VCF_SAMPLES),GEN[$i].$(gen)))

VCF_PAIRED_SAMPLES ?= 0 1
VCF_PAIRED_INFO_FIELDS = DP SS SSC GPV SPV CDA KGValidated OM PM GMAF
VCF_PAIRED_GEN_IDS ?= GT GQ DP RD AD FREQ DP4
VCF_PAIRED_GEN_FIELDS = $(foreach gen,$(VCF_PAIRED_GEN_IDS),$(foreach i,$(VCF_PAIRED_SAMPLES),GEN[$i].$(gen)))
ALL_VCF_PAIRED_FIELDS = $(VCF_FIELDS) $(VCF_PAIRED_INFO_FIELDS) $(VCF_PAIRED_GEN_FIELDS)

# If GATK VariantAnnotator is used, then use the SNPEFF_FIELDS Variable
SNPEFF_FIELDS = SNPEFF_EFFECT SNPEFF_FUNCTIONAL_CLASS SNPEFF_IMPACT SNPEFF_GENE_BIOTYPE SNPEFF_GENE_NAME SNPEFF_TRANSCRIPT_ID SNPEFF_AMINO_ACID_CHANGE SNPEFF_CODON_CHANGE
EFF_FIELDS = \"EFF[*].EFFECT\" \"EFF[*].IMPACT\" \"EFF[*].FUNCLASS\" \"EFF[*].CODON\" \"EFF[*].AA\" \"EFF[*].GENE\" \"EFF[*].BIOTYPE\" \"EFF[*].CODING\" \"EFF[*].TRID\" \"EFF[*].RANK\"

#----------
# Flags to Indicate how VCF Files Should be Annotated
# This is useful in situations where an a species may not have a dbsnp or cosmic
#----------
DBSNP_ANNOTATION_FLAG = true
COSMIC_ANNOTATION_FLAG = true

# Suffix Determines How to Annotate
VARSCAN_ANNOTATE_SUFFIX = .

ifeq ($(DBSNP_ANNOTATION_FLAG),true)
  VARSCAN_ANNOTATE_SUFFIX := $(VARSCAN_ANNOTATE_SUFFIX)snps_annotated.

  # Fields Only Available for dbsnp Annotations
  VCF_INFO_FIELDS += CDA KGValidated OM PM GMAF
endif

ifeq ($(COSMIC_ANNOTATION_FLAG),true)
  VARSCAN_ANNOTATE_SUFFIX := $(VARSCAN_ANNOTATE_SUFFIX)cosmic_annotated.
endif

ALL_VCF_FIELDS = $(VCF_FIELDS) $(VCF_INFO_FIELDS) $(VCF_GEN_FIELDS)
...
.SECONDARY: $(foreach sample,$(SAMPLES),$(foreach chr,$(CHROMOSOMES),varScan/vcf/$(sample).$(chr).varScan.indels.vcf)) $(foreach sample,$(SAMPLES),$(foreach chr,$(CHROMOSOMES),varScan/vcf/$(sample).$(chr).varScan.snps.vcf))
